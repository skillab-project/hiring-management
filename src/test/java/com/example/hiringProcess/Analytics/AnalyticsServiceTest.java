...
class AnalyticsServiceTest {
    ...
    private JobAdService repo2;
    private DepartmentService repo3;
    private CandidateService repo4;

    @BeforeEach
    void setUp() {
        repo = mock(AnalyticsRepository.class);
        service = new AnalyticsService(repo, repo2, repo3, repo4);
    }
...
//
//        JobAdStatsDto dto = service.getJobAdStats(jobAdId, 1);
//
...
//
//        StepStatsDto dto = service.getStepStats(jobAdId, stepId, 1);
//
...
//    }
    ...
    void getSkillStats_shouldComputePassRateAndHistogramFrom0to10() {
        ...
        when(repo.avgScoreForSkill(skillId,1)).thenReturn(7.777); // -> 7.8

        // αν θες null: Arrays.asList(4.9, 5.0, 10.0, null)
        when(repo.candidateSkillAverages(skillId,1)).thenReturn(List.of(4.9, 5.0, 10.0));

        SkillStatsDto dto = service.getSkillStats(skillId,1);
