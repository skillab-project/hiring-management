...
public class AnalyticsRepository {
    ...
    public long countTotalCandidatesByOrg(int orgId) {
        String sql = """
                  ...
                  JOIN departments d        ON d.id = jd.department_id
    ...
    public List<SkillAvgDto> topSkillsByOrg(int orgId, int limit) {
        String sql = """
                ...
                SELECT s.name AS skill, AVG(cs.avg_per_candidate) AS avg_score
                FROM cand_skill cs
    ...
    // Μετρά ΟΛΟΥΣ τους υποψήφιους ενός department με συγκεκριμένο status (όποιο
    // status του δώσoυμε στο όρισμα status)
    ...
    // Υπολογίζει τον μέσο αριθμό υποψηφίων ανά αγγελία εργασίας (job_ad) για το
    // συγκεκριμένο department.
    ...
    // Επιστρέφει histogram κατανομής βαθμών (buckets 0–9) ανά department (0–100
    // ranges)
    ...
    public List<OccupationAvgDto> occupationDifficultyByDept(int deptId) {
        String sql = """
                ...
                JOIN occupations oc   ON oc.id = j.occupation_id
    ...
    // Μετρά τους υποψήφιους με συγκεκριμένο status στο συγκεκριμένο department &
    // occupation.
    ...
    // Υπολογίζει τον Μ.Ο. υποψηφίων ανά αγγελία για το συγκεκριμένο department &
    // occupation
    ...
    // Μ.Ο. τελικού score υποψηφίου σε ΚΛΙΜΑΚΑ 0–100 για dept+occupation (skills
    // only)
    ...
    // Υπολογίζει τον μέσο βαθμό ανά question της αγγελίας (χαμηλότερο =
    // δυσκολότερο)
    ...
    public List<SkillAvgDto> skillDifficultyByJobAd(int jobAdId) {
        String sql = """
                ...
                SELECT sk.name  AS skill, AVG(cs.score100) AS avg_score
                FROM cand_skill cs
    ...
    public List<SkillAvgDto> candidateSkillScores(int candidateId) {
        String sql = """
                    ...
                    SELECT s.name AS skill, AVG(ss.score) * (SELECT sf100 FROM scale) AS avg_score
                    FROM skill_score ss
    ...
    // Παίρνει με ασφάλεια τιμή String από ResultSet για δεδομένη στήλη,
    // επιστρέφοντας null σε SQLException.
    ...
    // Συνενώνει ονοματεπώνυμο (first + last) καθαρίζοντας κενά και επιστρέφει null
    // αν το αποτέλεσμα είναι κενό.
    ...
    // Επιστρέφει τον μέσο όρο όλων των scores του συγκεκριμένου step, για το
    // συγκεκριμένο job ad
    ...
    // Επιστρέφει τον μέσο όρο (για το συγκεκριμένο step) ανά υποψήφιο του job ad –
    // 0–100
    ...
    public List<SkillAvgDto> skillRankingByStep(int jobAdId, int stepId) {
        String sql = """
                    ...
                    JOIN question q   ON q.step_id = st.id
                    JOIN question_skill qsk ON qsk.question_id = q.id
                    JOIN skills s      ON s.id = qsk.skill_id
                    JOIN candidate c  ON c.job_ad_id = ja.id
                    JOIN skill_score ss ON ss.candidate_id = c.id
                                       AND ss.skill_id     = qsk.skill_id
                                       AND ss.question_id  = q.id
    ...
    // Επιστρέφει το μέσο score της συγκεκριμένης ερώτησης στο context ενός job ad
    // (0–100),
    ...
    // Buckets κατανομής 0–9,10–19,...,90–100 (τελευταίος inclusive στο 100) για τη
    ...
    // Πόσοι υποψήφιοι έχουν score ≥ 50% στη συγκεκριμένη ερώτηση (μόνο από
    // skill_score).
    ...
    // Επιστρέφει τον μέσο όρο του σκορ της δεξιότητας σε όλο το dataset (όλα τα
    // question scores που την αφορούν)
    public Double avgScoreForSkill(int skillId, int orgId) {
        String sql = """
            SELECT COALESCE(AVG(x.sc), 0.0) AS avg_skill
            FROM (
                -- 1. Scores from formal Question Scores
                ...
                -- 2. Scores from raw Skill Scores (normalized / 10)
                ...
                WHERE k.skill_id = :sid 
                  ...
                  AND NOT EXISTS (
                      ...
                      JOIN step_score sr        ON sr.id = qs.step_score_id
                      JOIN interview_report ir  ON ir.id = sr.interview_report_id
                      JOIN candidate c2         ON c2.interview_report_id = ir.id
                      WHERE qs.question_id = ss.question_id
                        AND c2.id          = ss.candidate_id
                  )
            ) x
            """;
        Double v = jdbc.queryForObject(sql, Map.of("sid", skillId, "orgId", orgId), Double.class);
        return v == null ? 0.0 : v;
    }

    // Επιστρέφει τον μέσο όρο της δεξιότητας ανά υποψήφιο (για pass rate &
    // histogram)
    public java.util.List<Double> candidateSkillAverages(int skillId, int orgId) {
        String sql = """
            WITH unioned AS (
            ...
            GROUP BY cid
            """;
        return jdbc.query(sql, Map.of("sid", skillId, "orgId", orgId),
    ...
    public Double avgScoreForSkillInJobAdQuestion(int jobAdId, int questionId, int skillId) {
        String sql = """
                    ...
                    WHERE c.job_ad_id = :jobAdId
                      ...
                      AND ss.skill_id = :sid
                """;
        Double v = jdbc.queryForObject(sql, Map.of("jobAdId", jobAdId, "qid", questionId, "sid", skillId),
                Double.class);
        return v == null ? 0.0 : v;
    }
